...
void setup() {
    ...
    // Allow up to 30s to associate on busy APs
    ...
    if (WiFi.status() == WL_CONNECTED) {
        Serial.printf("[OTA] WiFi connected to '%s'  IP: %s\n", WIFI_SSID, WiFi.localIP().toString().c_str());
        ...
        ArduinoOTA.begin();
        Serial.printf("[OTA] Ready as '%s'  mDNS: %s.local\n", HOSTNAME, HOSTNAME);
        Serial.println("[OTA] Tip: In PlatformIO, set upload_port to the device IP if mDNS is unreliable.");
    } else {
